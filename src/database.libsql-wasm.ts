...
import { LibsqlError, type Client as LibSQLClient } from '@libsql/client-wasm';

type TransactionClient<DbSchema extends Record<string, unknown>> = LibSQLTransaction<
  DbSchema,
  ExtractTablesWithRelations<DbSchema>
>;

type Client<DbSchema extends Record<string, unknown>> = LibSQLDatabase<DbSchema> & {
  $client: LibSQLClient;
};
